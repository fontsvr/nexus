...
# ~ 31/7/25
# ~ useragent = "Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.169 Safari/537.36"
useragent = "Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.7258.66 Safari/537.36"
...
def downloadpage(url, post=None, headers=None, timeout=None, follow_redirects=True, cookies=True, replace_headers=False,
    ...
    if config.get_setting('httptools_timeout_searching'):
        timeout = config.get_setting('httptools_timeout_searching')

    logger.info("---------- Balandro: " + __version  + ' Page ----------')

    if use_proxy: logger.info("Proxy: %s" % use_proxy)

    logger.info("Time Out: %s" % timeout)
    logger.info("Url: " + url)
