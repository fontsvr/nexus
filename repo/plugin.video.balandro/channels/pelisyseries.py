...
def findvideos(item):
    ...
    for match in matches:
        ...
        itemlist.append(Item( channel = item.channel, action = 'play', url = url, server = servidor, title = '',
                              language = lang, quality = qlty, other = size, age = age ))
...
def normalize_server(server):
    ...
    server = server.replace('.org', '').replace('.net', '').replace('.club', '').replace('.site', '').replace('.watch', '').replace('.life', '').replace('.biz', '').replace('.fun', '').replace('.xyz', '').strip()
    ...
    elif 'steamplay' in server: server = ''
