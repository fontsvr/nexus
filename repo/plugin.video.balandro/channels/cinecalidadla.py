...
def do_downloadpage(url, post=None, headers=None):
    ...
    hay_proxies = False
    if config.get_setting('channel_cinecalidadla_proxies', default=''): hay_proxies = True

    if not url.startswith(host):
        data = httptools.downloadpage(url, post=post, headers=headers, raise_weberror=raise_weberror).data
    else:
        if hay_proxies:
            data = httptools.downloadpage_proxy('cinecalidadla', url, post=post, headers=headers, raise_weberror=raise_weberror).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers, raise_weberror=raise_weberror).data
...
def acciones(item):
    ...
    itemlist.append(Item( channel='helper', action='show_help_cinecalidadla', title='[COLOR aquamarine][B]Aviso[/COLOR] [COLOR green]Información[/B][/COLOR] canal', thumbnail=config.get_thumb('cinecalidadla') ))

    itemlist.append(Item( channel='actions', action='show_old_domains', title='[COLOR coral][B]Historial Dominios[/B][/COLOR]', channel_id = 'cinecalidadla', thumbnail=config.get_thumb('cinecalidadla') ))
...
def findvideos(item):
    ...
    if '>DESCARGAR<' in data:
        ...
        for url, servidor in matches:
            ...
            if servidor == 'torrent':
                qltys = scrapertools.find_multiple_matches(bloque, '<a href="(.*?)"(.*?)</li>')

                qlty = '?'

                for _url, _qlty in qltys:
                    if not _url == url: continue
